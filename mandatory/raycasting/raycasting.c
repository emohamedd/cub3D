...
/*   Updated: 2023/11/13 13:08:10 by houattou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_all_data	*draw_rays(t_all_data *data, int id, float ray_angle)
{
	...
	if (data->player->dis_v > data->player->dis_h)
	{
		if (data->player->is_ray_facing_up == TRUE)
		{
			...
			draw_wall_with_texture(data, id, ray_angle, data->cord->xstep,
				data->cord->ystep, xtx, data->textrs.no_texture);
		}
		else
		{
			
			draw_wall_with_texture(data, id, ray_angle, data->cord->xstep,
				data->cord->ystep, xtx, data->textrs.so_texture);
	...
	else
	{
		if (data->player->is_ray_facing_left == TRUE)
		{
			...
			draw_wall_with_texture(data, id, ray_angle, data->cord->xstep,
				data->cord->ystep, xtx, data->textrs.we_texture);
		}
		else
		{
			draw_wall_with_texture(data, id, ray_angle, data->cord->xstep,
				data->cord->ystep, xtx, data->textrs.ea_texture);
		}
	}
	return (data);
}
...
void	start_raycasting(t_all_data *data)
{
	...
	id = 0;
	num_of_rays = WIDTH;
	ray_angle = normalize_angle(data->player->rotation_angle - (FOV_ANGLE / 2));
