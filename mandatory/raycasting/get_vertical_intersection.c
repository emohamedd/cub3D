...
/*   Updated: 2023/11/12 22:16:35 by houattou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_vertical_intersection(t_all_data *mlx, float ray_angle)
{
	...
	while (true)
	{
		if ((is_ray_facing_left && is_has_wall(mlx, mlx->cord->xstep_v
					- SIZE_TITLE, mlx->cord->ystep_v)) || (!is_ray_facing_left
				&& is_has_wall(mlx, mlx->cord->xstep_v, mlx->cord->ystep_v))
			|| (is_ray_facing_left && is_has_wall(mlx, mlx->cord->xstep_v,
					mlx->cord->ystep_v)))
		{
			break ;
		}
