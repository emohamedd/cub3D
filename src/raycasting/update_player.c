...
/*   Updated: 2023/11/11 18:01:10 by houattou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void update_player(t_all_data *data, mlx_key_data_t keydata )
{
    int sign;
    float speed;
    ...
    sign = 1;
    direction_angle = data->player->rotation_angle;
    if (keydata.key == MLX_KEY_S || keydata.key == MLX_KEY_A)
			sign = -1;
	if (keydata.key == MLX_KEY_D ||keydata.key == MLX_KEY_A)
		direction_angle += M_PI / 2;
    new_x = data->player->x + sign*data->player->move_speed * cos(data->player->rotation_angle);
    new_y = data->player->y + sign * data->player->move_speed * sin(data->player->rotation_angle); 
    if( (check_if_have_wall(data, new_x, new_y)) == TRUE)
    {
