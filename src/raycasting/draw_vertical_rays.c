...
/*   Updated: 2023/10/27 20:53:06 by houattou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_all_data*	draw_vertical_intersection(t_all_data *mlx, float ray_angle)
{
	...
	while ((mlx->cord->xstep_v >= 0 && mlx->cord->xstep_v < WIDTH && mlx->cord->ystep_v >= 0 && mlx->cord->ystep_v < HEIGHT))
	{
		...
		(is_ray_facing_left && is_has_wall(mlx, mlx->cord->xstep_v, mlx->cord->ystep_v)) || (!is_ray_facing_left && is_has_wall(mlx, mlx->cord->xstep_v - SIZE_TITLE, mlx->cord->ystep_v)))
		{
